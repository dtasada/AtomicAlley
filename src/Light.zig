...
pub fn init(position: rl.Vector3, target: rl.Vector3, color: rl.Color, intensity: f32, shader: rl.Shader) Self {
    var light: Self = .{
        .enabled = true,
        .position = position,
        .target = target,
...
pub fn updateLights(camera: *rl.Camera, light_shader: rl.Shader, lights: *const std.ArrayList(Self)) void {
    ...
    for (lights.items) |l| {
        rl.drawSphere(l.position, 1.0, l.color);
        l.update(light_shader);
    }
}
